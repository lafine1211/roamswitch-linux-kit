...
- `security_report()` / `server_security_report()` — full local audit; the latter runs the Server Edition's 27-check profile (kernel hardening, container isolation, kernel CVE exposure, eBPF LSM) instead of the desktop client's 24 checks.
